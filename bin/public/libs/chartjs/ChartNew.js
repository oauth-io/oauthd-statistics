...
function doMouseMove(ctx, config, event) {
    ...
    for (i = 0; i < jsGraphAnnotate[ctx.ChartNewId]["length"]; i++) {
        ...
        if (jsGraphAnnotate[ctx.ChartNewId][i][0] == "ARC") // Arc
        {
            ...
            // console.log("jsGraphAnnotate[ctx.ChartNewId][i][1]",jsGraphAnnotate[ctx.ChartNewId][i][1]);
            // console.log("jsGraphAnnotate[ctx.ChartNewId][i][2]",jsGraphAnnotate[ctx.ChartNewId][i][2]);
            distance = Math.sqrt((canvas_pos.x - jsGraphAnnotate[ctx.ChartNewId][i][1]) * (canvas_pos.x - jsGraphAnnotate[ctx.ChartNewId][i][1]) + (canvas_pos.y - jsGraphAnnotate[ctx.ChartNewId][i][2]) * (canvas_pos.y - jsGraphAnnotate[ctx.ChartNewId][i][2]));
            // console.log("distance", distance);
            if (distance < 10) {
                // console.log("LITTLE distance", distance);
                ...
                dispString = tmplbis(config.annotateLabel, { config:config, v1: v1, v2: v2, v3: v3, v4: v4, v5: v5, v6: v6, v7: v7, v8: v8, v9: v9, v10: v10, v11: v11, v12: v12, graphPosX: graphPosX, graphPosY: graphPosY });
                // console.log("config.annotateLabel", config.annotateLabel);
...
//        pieRadius = Min([msr.availableHeight / 2, msr.availableWidth / 2]) - 5;
    ...
    var Line = function (data, config, ctx) {
        ...
        // console.log("msr.leftNotUsableSize", msr.leftNotUsableSize);
        // console.log("config.scaleTickSizeLeft", config.scaleTickSizeLeft);
                ...
                for (var j = 0; j < data.datasets[i].data.length; j++) {
                    if (!(typeof(data.datasets[i].data[j])=='undefined')) {
                      ...
                      if (animPc >= 1) {
                        ...
                        // console.log("xPos(j)",xPos(j));
                        // console.log("yPos(i, j)",yPos(i, j));
...
//                ctx.strokeStyle = data.datasets[i].strokeColor;
                if (animPc >= 1) {
                 ...
                 for (var j = 0; j < data.datasets[i].data.length; j++) {
                     ...
                     if(typeof(data.datasets[i].fillColor)=="string"){ctx.fillStyle = data.datasets[i].fillColor;}
                     else if(typeof(data.datasets[i].fillColor[0])=="string"){ctx.fillStyle = data.datasets[i].fillColor[Min([data.datasets[i].fillColor.length,j])];}

                     if(typeof(data.datasets[i].strokeColor)=="string"){ctx.strokeStyle = data.datasets[i].strokeColor;}
                     else if(typeof(data.datasets[i].strokeColor[0])=="string"){ctx.strokeStyle = data.datasets[i].strokeColor[Min([data.datasets[i].strokeColor.length,j])];}
        ...
        if (drawAxis && (config.xAxisBottom || config.xAxisTop)) {
            ...
            if (config.rotateLabels == "smart") {
                ...
                if ((availableWidth + config.xAxisSpaceBetweenLabels) / nblab < (widestLabel + config.xAxisSpaceBetweenLabels)) {
                    rotateLabels = 45;
                    // console.log("config.spaceLeft", config.spaceLeft);
                    // console.log("availableWidth", availableWidth);
                    // console.log("availableWidth / nblab", (availableWidth + config.spaceLeft) / nblab);
                    // console.log("Math.abs(Math.cos(rotateLabels * Math.PI / 180) * widestLabel)", Math.abs(Math.cos(rotateLabels * Math.PI / 180) * widestLabel));
                    if ((availableWidth + config.spaceLeft) / nblab < Math.abs(Math.cos(rotateLabels * Math.PI / 180) * widestLabel)) {
                        rotateLabels = 90;
                    }
                }
            } else {
