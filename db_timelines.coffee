...
module.exports = (shared) ->
	...
	exp = {}
	exp.getTotal = check 'string', (target, callback) ->
		db.redis.get 'st:' + target + ':t', (err, total) ->
			return callback err if err
			callback null, total

	exp.getTimeline = check 'string', unit:'string', start:['int','number'], end:['int','number'], (target, data, callback) ->
		unit = data.unit || 'm'
		keys = {}
		date = new Date()
	...
	exp.addUse = check target:'string', uses:['number','none'], (data, callback) ->
		date = new Date
